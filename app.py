...
payment_handler = PaymentHandler(bot, redis_client=redis_client)
...
# Modify your message handlers to check subscription
def check_user_access(message):
    """Check if user has access to bot features"""
    user_id = str(message.from_user.id)
    if payment_handler.check_subscription(user_id):
        return True

    payment_handler.send_payment_message(message.chat.id, user_id)
    return False
...
def handle_phone_number(message,recipient_name,bank_name,service_name):
    ...
    # Create Redis client
    # redis_client = create_redis_client() #Use the already available client.
...
# Add payment routes to Flask app
web_app.register_blueprint(init_payment_routes(payment_handler))
